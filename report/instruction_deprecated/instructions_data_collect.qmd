...
[x] You must use **at least one API** to collect your data.
[x] Ensure you have at least one **regression target**: a continuous quantity that can be used for regression prediction with other features. (year)
[x] Ensure you have at least one **binary classification target**: a two-class (A,B) label that can be predicted using other features. 
[x] Ensure you have at least one **multiclass-classification target**: a multi-class (A,B,C ...) label that can be predicted using other features.
...
1. **Data Source Information**  
   - [x] Specify where the data comes from, whether public datasets, APIs, or original collection.
   - [x] Provide links to datasets or API documentation and explain their relevance to the project.

2. **Data Collection Methods**  
   - [x] If you used an API or web scraping, explain the tools (e.g., `BeautifulSoup`, `Selenium`) and the process.
   - [x] Include links to your code for replicating data collection.

3. **Data Structure and Format**  
   - [x] Describe the structure of your raw data (e.g., rows, columns, data types) and its format (CSV, JSON, etc.).

4. **Linking to Data**  
   - [x] Include links to the **raw dataset**, ensuring transparency and allowing replication.

5. **Code and Reproducibility**  
   - [x] Link to all relevant code, including data collection , with clear documentation to enable replication.

6. **Transparency and Replicability**  
   - [x] Ensure your entire workflow is transparent and reproducible by linking all data, code, and documentation. This allows others to follow your process from start to finish.
